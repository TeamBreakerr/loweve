...
<template>
  <main class="page game-subpage">
    ...
    <div class="section__head"><span class="section__hint">玩过 {{ marks.list.length }} 款</span><select v-model="sortMode" class="sort-select" aria-label="排序方式"><option value="recent">最近添加</option><option value="rating">按评分</option></select><button class="btn btn--rose" @click="addOpen=true"><svg class="btn__ic" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>添加游戏</button></div>
    <p v-if="marks.loading" class="game-page-state">加载中…</p><p v-else-if="!marks.list.length" class="game-page-state">还没有游戏记录。</p>
    <div v-else class="game-library-grid"><article v-for="item in sortedMarks" :key="item.id" class="game-library-card"><div class="game-library-card__poster"><GamePoster :url="item.work.cover_url" :fallback="item.work.header_url" :state="item.work.release_state" @click="router.push(`/games/work/${item.work_id}`)"/><button class="game-library-card__edit" @click="edit(item)">✎</button></div><h3 @click="router.push(`/games/work/${item.work_id}`)">{{ item.work.title }}</h3><div class="game-library-card__record"><GameContentBadge :work="item.work"/><span v-if="item.rating" class="game-my-score">我 {{ item.rating }}</span></div><div class="game-library-card__market"><GameRating :work="item.work" compact/><GamePrice :work="item.work"/></div></article></div>
...
/* 排序下拉：原生 select 无基础重置，按游戏区主题色收成胶囊（沿用本页 hint→按钮 的左排布局） */
.sort-select{ font-family:inherit; font-size:var(--fs-sm); color:var(--text-dim); background:var(--surface); border:1px solid var(--game-line); border-radius:var(--r-pill); padding:6px 12px; cursor:pointer; outline:none; }
.sort-select:hover, .sort-select:focus-visible{ color:var(--text); border-color:var(--game-accent); }
</style>
