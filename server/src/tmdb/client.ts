...
export function createTmdbClient({ token, key, resolve, fetch = globalThis.fetch, retryDelays = DEFAULT_RETRY_DELAYS }: { token?: string | null; key?: string | null; resolve?: () => any; fetch?: any; retryDelays?: number[] } = {}) {
  ...
  return {
    ...
    movieDetail(id: any) {
      return request(`/movie/${id}`, { append_to_response: 'external_ids,alternative_titles,translations,credits,videos' });
    },

    tvDetail(id: any) {
      return request(`/tv/${id}`, { append_to_response: 'external_ids,alternative_titles,translations,credits,videos' });
    },
