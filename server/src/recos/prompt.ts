...
export function buildMessages({ userA, userB, marksA, marksB, sessions, plan, avoidTitles, userPrompt }: any) {
  ...
  const watchedA = marksA.filter((m: any) => m.status === 'watched' && markHasSignal(m));
  const watchedB = marksB.filter((m: any) => m.status === 'watched' && markHasSignal(m));
  ...
  const lines = [
    ...
    `# 避雷池（这些别再推荐，已看过或已明确不感兴趣）：${avoidTitles.length ? avoidTitles.map((t: any) => `《${t}》`).join('、') : '（无）'}`,
  ];
