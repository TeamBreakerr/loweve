...
//   2. 影视与游戏首页的横向轨道都不做 scroll-snap —— 吸附会把每一次滚动都拽成整卡跳格，
//      手感顿挫（用户报的「滑动以海报边界对齐」）。
...
// 滚一个跟卡片宽度无关的零头：有吸附时浏览器会把它拽到下一张卡的边界，没吸附就原地停住。
const railExpression = `(async () => {
  ...
  for (const viewport of document.querySelectorAll('.horizontal-rail__viewport')) {
    ...
    rails.push({
      ...
      scrollLeftAfter37: Math.round(viewport.scrollLeft),
    });
    viewport.scrollLeft = 0;
  }
  return rails;
...
try {
  ...
  for (const [width, height] of [[2400, 1838], [1512, 982], [900, 1000], [420, 900]]) {
    ...
    for (const rail of rails) {
      if (rail.snapType !== 'none') errors.push(`${width}px：轨道「${rail.label}」仍在吸附（scroll-snap-type=${rail.snapType}）`);
      if (rail.childSnapAlign && rail.childSnapAlign !== 'none') {
        errors.push(`${width}px：轨道「${rail.label}」卡片仍有 scroll-snap-align=${rail.childSnapAlign}`);
      }
      if (rail.scrollLeftAfter37 !== 37) {
        errors.push(`${width}px：轨道「${rail.label}」滚 37px 后停在 ${rail.scrollLeftAfter37}px（被吸附到卡片边界）`);
      }
    }
  }
  ...
  for (const rail of gameRails) {
    if (rail.snapType !== 'none') errors.push(`游戏页：轨道「${rail.label}」仍在吸附（scroll-snap-type=${rail.snapType}）`);
    if (rail.scrollLeftAfter37 !== 37) {
      errors.push(`游戏页：轨道「${rail.label}」滚 37px 后停在 ${rail.scrollLeftAfter37}px（被吸附到卡片边界）`);
    }
  }
